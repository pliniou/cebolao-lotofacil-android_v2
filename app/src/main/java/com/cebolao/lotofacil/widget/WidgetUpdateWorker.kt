...
class WidgetUpdateWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result = runCatching {
        ...
        val synced = runCatching { historyRepository.syncHistory().join() }.isSuccess
        if (synced) {
            updateLastDrawWidgets()
            updateNextContestWidgets()
        }
    ...
    private suspend fun updateNextContestWidgets() {
        ...
        ids.forEach { id ->
            ...
            if (details != null && details.nextContestNumber != null) {
                applyNextContestContent(
                    small,
                    title = "${context.getString(R.string.widget_next_contest_title_generic)} ${details.nextContestNumber}",
                    date = details.nextContestDate ?: DEFAULT_PLACEHOLDER,
                    prize = Formatters.formatCurrency(details.nextEstimatedPrize)
                )
