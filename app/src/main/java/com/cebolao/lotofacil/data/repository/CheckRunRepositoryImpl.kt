...
class CheckRunRepositoryImpl @Inject constructor(
    ...
    private fun CheckRunEntity.toCheckReport(json: Json): CheckReport? {
        return try {
            ...
            // Reconstruir LotofacilGame a partir do bitmask
            val ticket = com.cebolao.lotofacil.domain.model.LotofacilGame.fromMask(ticketMask)
            
            // Reconstruir hits a partir do histórico (não armazenamos hits diretamente, apenas o hash)
            // Por enquanto, retornamos lista vazia - podemos melhorar isso no futuro
            val hits = emptyList<com.cebolao.lotofacil.domain.model.Hit>()
